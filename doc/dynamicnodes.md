...
But you should have at least 2 Raspberry Pi 5 boards to test dynamic node setup.

### Network configuration

Your network configuration (router setting for example) should match the below requirements:
...
You can change the IP address and build main/secondary images yourself: update static IP and gateway IP in [yaml](../aos-rpi.yaml) file.

### Flash AosCore install image to SD card using Raspberry Pi Imager

This example uses 2 RPI5 with nVME. Follow the steps described in the [README.md](../README.md#flash-aoscore-install-image-to-sd-card-using-raspberry-pi-imager) to prepare SD cards for the main and secondary nodes:

1. `AosCore main node for NVMe drive` - for the RPI board that will be main (SD card #1)
2. `AosCore secondary node for NVMe drive` - for the RPI board that will be secondary (SD card #2)
...
* RPI5 board with main node image contains: main-dom0, main-domd nodes;
* RPI5 board with secondary node image contains: secondary-dom0, secondary-domd nodes;

*Note*: dynamic nodes provisioning requires less steps compared to [README.md](../README.md#provision-device-with-aoscloud), because our `main` node is configured with static ip.

### Known issues:
* `Zephyr RTOS` Dom0 freezes in rare cases after provisioning. Node works fine after rebooting RPI5 board with the frozen Dom0 node.
